...
class CheckTokenTest extends PassportTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        Route::get('/bearer', fn () => 'response')->middleware(CheckToken::using('foo', 'bar'));
        Route::get('/guarded', fn () => 'response')->middleware(['auth:api', CheckToken::using('foo', 'bar')]);
    }

    public function test_request_is_passed_along_if_token_has_all_required_scopes()
    {
        $token = $this->user->createToken('test', ['foo', 'bar'])->accessToken;
    ...
    public function test_request_is_forbidden_if_token_is_missing_a_required_scope()
    {
        $token = $this->user->createToken('test', ['foo'])->accessToken;
